...
than 2 million unique visitors every month.
...
To accommodate the need for good ACE algorithms, the MIREX audio chord
estimation task has received a lot of attention within the MIR community. Since
its inception in 2008, each year between 6 and 13 different teams have submitted
in total 11 to 18 different algorithms. Although ACE algorithms are applied
within industry and academia, and the number of participants has decreased
slightly the last three years, the problem of automatic chord transcription is
nowhere near solved. On fresh validation data, the best-performing algorithms in
2013 barely reached a 75 percent correct overlap when considering only 24 major
and minor chords.
...
harmony. Furthermore, it was consent that next to a measure of correct overlap
the segmentation of a chord sequence should be measured properly.

At approximately the same time Christopher Harte proposed new solutions to
measure the chord sequence recall and segmentation that solved the problems
indicated in the Utrecht agreement~\cite{harte2010}. \Bas{Maybe we should
explain Harte's work in more detail here.} Recently, Pauwels en
Peeters~\cite{Pauwels-Peeters-2013} implemented most of the evaluation
improvements suggested by Harte and performed a rigorous re-evaluation of all
MIREX ACE submissions in 2010--2012. Al last, at ISMIR
2013\footnote{\url{http://www.music-ir.org/mirex/wiki/2013:Audio_Chord_Estimation}}
also the evaluation of chord sequence segmentation was addressed and implemented
suggested by Harte~\cite{harte2010} and Mauch~\cite{mauch2010}.
