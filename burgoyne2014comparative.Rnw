...
\threeauthors
  ...
  {Johan Pauwels} {STMS IRCAM-CNRS-UPMC \\ {\tt
      johan.pauwels@gmail.com}\thanks{Johan Pauwels is no longer
      affiliated with IRCAM. Data and source code for this paper, including
    that necessary to reproduce all statistics and figures, is
    available from \protect\url{http://bitbucket.org/jaburgoyne/ismir-2014}.}}
...
$B_{jj} = \var\left[P_j \mid N, \vec{X}_j\right]$,
\begin{equation}
  \label{eq:gee-cov}
  \cov\left[P \mid N, \vec{X}\right] =
