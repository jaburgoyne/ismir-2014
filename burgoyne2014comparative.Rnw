...
\threeauthors
  ...
  {Johan Pauwels} {IRCAM \\ {\tt johan.pauwels@gmail.com}}
...
\acro{MIREX} results from last year 
\Bas{I would say: ``2013", as the ``last year'' will change on a regular basis}%
...
\acro{CSR} directly. In the interest of reproducible research, we used
...
interaction. The logistic regression model, in contrast, identified a
...
different order than the headline \acro{WCSR}-based results.
...
of all chords under Vocabulary V. Furthermore, although \acro{KO1} can
recognise seventh chords in theory, it was mostly likely trained on
the relatively seventh-poor Isophonics corpus (only 15 percent of all
chords); \acro{KO2} is the same algorithm trained locally on an
