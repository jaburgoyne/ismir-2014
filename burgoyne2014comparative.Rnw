...
## TODO: Ideally, these would be weighted boxplots, too. 
...
\begin{figure*}
  ...
  \caption{Boxplots and compact letter displays for the \acro{MIREX}
    ...
    .005. Although Friedman's \acro{ANOVA} detects
    \Sexpr{sum(friedman.summary$test$pvalues < .005) -
      sum(logistic.summary$test$pvalues < .005)} more significant
    pairwise differences than logistic regression
    (\Sexpr{sum(friedman.summary$test$pvalues < .005)}
