...
\threeauthors
  ...
  {Johan Pauwels} {STMS IRCAM-CNRS-UPMC\footnote{The author no longer works here} \\ {\tt johan.pauwels@gmail.com}}
...
\begin{abstract}
  ...
  used a new evaluation scheme. 
  \Bas{I'm not the native speaker here ;-), but can you really say this:
  ``a task \dots used''? If so, fine, otherwise I'd suggests something like:
  ``In the 2013 audio chord estimation task \acro(ACE) a new evaluation scheme
  was used.''}%
  Using chord vocabularies of differing
...
music teaching tools, and at the time of writing, Chordify attracts more
...
\acro{(MIREX)} \acro{ACE} task was established as an
...
participants has decreased slightly over the last three years, the problem
...
range of possible chords was restricted exclusively to the 25 major,
minor and ``no-chord'' labels; the performance figure drops to 60 percent when the
...
\begin{table*}
 ...
 \begin{tabular}{clcD{.}{}{2.0}ccD{.}{}{2.0}D{.}{}{2.0}D{.}{}{2.0}D{.}{}{2.0}D{.}{}{2.0}D{.}{}{2.0}D{.}{}{2.0}D{.}{}{2.0}}
  ...
  \caption{Number of supported chord types, inversion support,
    ...
    \acro{MIREX} Wiki.}
    \Bas{If we really need the space, this table could be fit into 
    one column}
...
that the evaluation rubrics\Johan{Now there's an English word I've never heard before, maybe you could simplify this for me and other non-native speakers, Ashley} could be improved. At that time, the
...
chords, 12 minor chords and a ``no-chord'' label. 
At \acro{ISMIR} 2010, a group of ten researchers met
...
vocabularies, such as seventh chords and inversions, as the 25-chord
...
\Johan{Good point, a quick search learns me that DHD seems to be the already established name and it's not that new in fact. It comes from the image segmentation field and has been first used for music in Abdallah et al. 2005 ISMIR paper. See also http://ismir2008.ismir.net/papers/ISMIR2008_219.pdf, we'd better also reattribute the citations on line 216.}
...
%% \Ashley{It turns out that you actually *can* do a weighted Friedman test, as it turns out. You just have to formulate as an (almost) normal two-way ANOVA with the variables in a particular order (and still with rank transformation, of course). I had no idea. Remind me the next time I see you and I'll show you.}
...
\acro{MIREX} results from 2013 
to illustrate four key benefits:
easier interpretation, greater statistical power, an inter-correlation
for identifying relationships among submitted algorithms, and better
detection of outliers.

\section{Logistic Regression with GEEs}\label{sec:logistic-regression}
\Bas{In this section the term 
$\pi(\vec{x}; \vec{\betaup}) [1 - \pi(\vec{x}; \vec{\betaup})]$
occurs frequently. Often $\betaup$ and $\vec{x}$ are correctly 
used in the formula, but not addressed specifically. I wonder whether
in some of the formulas it would not be clearer to leave out $\betaup$ and 
$\vec{x}$, and just write $\pi[1 - \pi]$.}
...
\begin{equation}
  ...
  \ .
\end{equation}
\Bas{I guess that $\triangleq$ means equal by definition? I haven't seen
the $\triangleq$ very frequently. Is it very common in the Logistic 
regression communities?}%
\Johan{Just as a second opinion, for me it is very well known as a mathematical notation I already encountered in secondary school, not specific to LR communities}
...
\begin{figure*}
  ...
  \caption{
  \Bas{Is it WCSR or CSR that is displayed here? (the caption on 
  the y-axis states CSR)}
  Boxplots and compact letter displays for the \acro{MIREX} \acro{ACE} 2013 results on the \emph{Billboard} 2013 test set with vocabulary V (seventh chords and inversions). $N =$ 161 songs per algorithm. Given the respective statistical models, there are insufficient data to distinguish among algorithms sharing a letter, correcting for multiple comparisons to hold the \acro{FDR} at $\alpha =$ .005. Although Friedman's \acro{ANOVA} detects slightly more significant pairwise differences than logistic regression (\Sexpr{sum(friedman.summary$test$pvalues < .005)} vs. \Sexpr{sum(logistic.summary$test$pvalues < .005)}), it operates on a different scale than \acro{CSR} and misorders algorithms relative to \acro{WCSR}.
\Bas{I compared the Friedman test results with the Matlab script that has 
been used in MIREX for ages and that I've adapted to MIREX ACE 2013. 
This Friedman analysis gives slightly different result. I guess this 
is because CSR is taken into account instead of WCSR? In ``the Matlab 
Friedman'' only 41 significant differences can be discerned.}  
  }
  \label{fig:cld-plots}  
\end{figure*}
...
$p_{ij}$ represents the \acro{CSR}\Johan{in the end, isn't it always WCSR that is used now that it is clear that the weighting can be handled?} or segmentation score for algorithm $j$
...
\begin{equation}
  ...
  \ .
\end{equation}
\Bas{There is one `)' to many in Equation~\ref{eq:gee-var}.}%
...
results of logistic regression on all combinations, and so we
will focus on a single one of the data sets, the \emph{Billboard} 2013
test set. In some cases, logistic regression allows us to speak to all
measures (11\,592 observations), but in general, we will also
restrict ourselves to discussing the newest and most challenging of
the harmonic vocabularies for \acro{WCSR}: Vocabulary V, which
includes major chords, minor chords, major sevenths, minor sevenths,
dominant sevenths, and the complete set of inversions of all of the
above (1932 observations). We are interested in four key questions.
\begin{enumerate}
...
\item Which algorithm-song pairs exhibited unexpectedly good or
  unexpectedly bad performance, and is there anything to be learned
...
The boxplots in Figure~\ref{fig:cld-plots} give a broader view of the
performance of each algorithm than the overall performance measures
from Table~\ref{tab:mirex-results}, and above the boxplots, all
significant pairwise differences are recorded as a \emph{compact
  letter display}. The figure is restricted to Vocabulary V, with the
algorithms in descending order by
...
\acro{CSR}\Johan{same remark about WCSR vs CSR} directly. In the interest of reproducible research, we used
a stricter $\alpha =$ .005 threshold for reporting pairwise
comparisons with the more contemporary false-discovery-rate
\acro{(FDR)} approach of Benjamini and Hochberg as opposed to more
traditional Tukey tests at $\alpha =$ .05
\cite{Benjamini-Hochberg-1995,Johnson-2013}. Within either of the
subfigures, the difference in \acro{WCSR} between two algorithms that
...
\begin{table*}
  ...
  \caption{Pearson's correlation matrix on the coefficients of the
    ...
    the \acro{FDR}. A set of algorithms stands out for
...
logistic regressions and looked for significant interactions between
algorithm and either inversion (present or absent from the vocabulary)
or complexity of the vocabulary (root only, major-minor, or
...
\Sexpr{FormatP(friedman.vocab.anova[5,4])}, but inversion was
significant neither as a main effect nor as part of any
interaction.\Johan{wouldn't it be clearer to say "insignificant both as a main effect and as ..."? The significant followed by a negation slightly bothers me :-)} The logistic regression model, in contrast, identified a
three-way Algorithm $\times$ Complexity $\times$ Inversions
interaction, $\chi^2($\Sexpr{logistic.vocab.anova[1,1]}$) =$
\Sexpr{round(logistic.vocab.anova[1,2], 2)},
\Sexpr{FormatP(logistic.vocab.anova[1,3])}.

\Johan{I would mention that only 1 algo supports inversions, now it's only mentioned in the table and not in the text (as far as I can see) and it's pretty relevant there with the algo x complexity interaction.}

\subsection{Correlation Matrices}

Table~\ref{tab:cor} presents the correlation matrix of the residuals
after logistic regression on \acro{WCSR} for Vocabulary V. Friedman's
\acro{ANOVA} has no useful analogue: all its entries are necessarily
the same. Significant correlations are marked, again using the
Benjamini--Hochberg procedure to control the \acro{FDR} at $\alpha =$
.005. Positive correlations do not necessarily imply that the
algorithms perform similarly; rather it implies that they find the
same songs relatively easy or difficult. Negative correlations imply
that songs that one algorithm finds difficult are relatively easy for
the other algorithm.
...
case. Figure~\ref{fig:correlations} summarises the original correlation
