...
vocabularies, such as seventh chords, inversions, etc., as the
vocabulary of twelve major and minor chords was considered a rather
coarse representation of tonal
...
\cite[pp.~900--01]{Kutner2005}. The rank transformation makes
...
There is no true need to pay the costs of Friedman's \acro{ANOVA} for
evaluating chord estimation, however. Fundamentally, \acro{WCSR} is a
probability, specifically the probability that an estimation algorithm
will label an arbitrary frame of audio correctly, and as such, it fits
naturally into a logistic regression (logit) model. Likewise, the new
segmentation score, also constrained to fall between 0 and 100
percent, is appropriate for logistic regression. The remainder of this
paper describes how exactly logistic regression can be used for
comparative evaluation of chord estimation algorithms and uses the
\acro{MIREX} results from last year to illustrate four key benefits of
this approach: easier interpretation, greater statistical power, an
inter-correlation for identify relationships among submitted
algorithms, and better detection of outliers.

\section{Logistic Regression with GEEs}

GEEs to control for random effects.
...
logistic.outliers <- with(billboard.2013, {
  logistic.fit <- geeglm(formula = performance ~ algorithm * analysis,
                         ...
                         corstr = "exchangeable",
                         weights = duration, id = song)
  billboard.2013[abs(resid(logistic.fit)) > chauvenet * sd(resid(friedman.fit)),]
})
@ 
